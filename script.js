...
const players = {
  ...
  playerTwo: { name: "April", wins: 0 },
};

let move=1;  // Count the number of moves, to thereby calculate the number of moves
let nextPlayer= players.playerOne.name; //This player starts the game as well as act as the next one in the middle of the game.
let pastPlayer;
let currentImage= "cross"; //Since player1 will start the game and it's symbol is cross, so to keep trackof that and keeping default value as cross.
let playerHasWon= false;  //For resetting the game, when any player wins the game.
...
function checkForTie() {
  ...
  const allSquaresFilled = squares.every((square) => {  //If all square field is filled, then it's a tie.
    return (
      square.classList.contains("cross") || square.classList.contains("circle")
    );
  });

  if (allSquaresFilled && !playerHasWon) {  //Beacuse if playerr wins in last move, all squares will be filled and we have to differentiate if it's a  tie or a win.
...
function restartGame() {  //To initially start the game without the delay of settimeout
  removeSquareClick();
  reset();
}
